...
public class Initializer implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent sce) {
        ...
        supplierDataStore.add(new Supplier("Amazon", "Digital content and services"));
        Supplier lenovo = null;
        supplierDataStore.add(new Supplier("Lenovo", "Computers"));
        Supplier samsung = null;
        supplierDataStore.add(new Supplier("Samsung", "Electronics"));
        Supplier sony = null;
        supplierDataStore.add(new Supplier("Sony", "Electronics"));
        Supplier apple = null;
        supplierDataStore.add(new Supplier("Apple", "Electronics"));
        Supplier lg = null;
        supplierDataStore.add(new Supplier("LG", "Electronics"));

        try {
            amazon = supplierDataStore.findByName("Amazon");
            lenovo = supplierDataStore.findByName("Lenovo");
            samsung = supplierDataStore.findByName("Samsung");
            sony = supplierDataStore.findByName("Sony");
            apple = supplierDataStore.findByName("Apple");
            lg = supplierDataStore.findByName("LG");
        } catch (SQLException e) {
            e.printStackTrace();
        }

        //setting up a new product category
        ProductCategory tablet = null;
        ProductCategory tv = null;
        ProductCategory phone = null;
        ProductCategory computer = null;
        productCategoryDataStore.add(new ProductCategory("Tablet", "Hardware", "A tablet computer, commonly shortened to tablet, is a thin, flat mobile computer with a touchscreen display."));
        productCategoryDataStore.add(new ProductCategory("TV", "Hardware", "A box which gives you the impression of actually seeing inside things."));
        productCategoryDataStore.add(new ProductCategory("Phone", "Hardware", "A calling device."));
        productCategoryDataStore.add(new ProductCategory("Computer", "Hardware", "Machine that will take over the world."));

        try {
            tablet = productCategoryDataStore.findByName("Tablet");
            tv = productCategoryDataStore.findByName("TV");
            phone = productCategoryDataStore.findByName("Phone");
            computer = productCategoryDataStore.findByName("Computer");
        } catch (SQLException e) {
            e.printStackTrace();
        }
